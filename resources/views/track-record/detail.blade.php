                    ...
                    <div class="col ">
                        <div class="row">
                            <div class="col ">
                                ...
                                <p style="color: black"><span class="iconify" data-inline="false" data-icon="ic:baseline-email" style="font-size: 40px;"></span>
                                    {{ $employee->email }}
                                </p>
                                <p style="color: black"><span class="iconify" data-inline="false" data-icon="bx:bxs-id-card" style="font-size: 40px;"></span>
    ...
    <div class="col-sm-auto">
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                ...
                <a class="btn btn-primary d-block inputPeriod" href="{{ url('track-record/insertTraining') }}" style="margin-bottom: 2%"><b>Tambahkan Data Pelatihan</b></a>
                <a class="btn btn-success d-block inputPeriod" href="{{ url('track-record/insertProject') }}"><b>Tambah Data Project</b></a>
            </div>
        </div>
    </div>
    @endif
            ...
            <table class="table table-bordered dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center"><a href="{{ url('track-record/training/' . $item->id) }}" class="btn btn-primary tr-{{$item->status}}">Lihat</a>
                            @if (session('permission') == 'user')
                            @if ($item->status == 'Menunggu' || $item->status == "Ditolak")
                            <a href="{{ url('track-record/training/edit/' . $item->id) }}" class="btn btn-warning">Edit</a>
                            <form action="{{ url('track-record/training/delete/' . $item->id) }}" method="post" class="d-inline" onsubmit="return confirm('Apakah Anda yakin untuk menghapus data ini?')">
                                @method('delete')
                                @csrf
            ...
            <table class="table table-bordered dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center"><a href="{{ url('track-record/project/' . $item->id) }}" class="btn btn-primary">Detail</a>
                            @if (session('permission') == 'user')
                            @if ($item->status == 'Sedang Berlangsung')
                            <a href="{{ url('track-record/project/edit/' . $item->id) }}" class="btn btn-warning">Edit</a>
                            <form action="{{ url('track-record/project/delete/' . $item->id) }}" method="post" class="d-inline" onsubmit="return confirm('Apakah Anda yakin untuk menghapus data ini?')">
                                @method('delete')
                                @csrf
                                <button class="btn btn-danger">Hapus</button>
                            </form>
