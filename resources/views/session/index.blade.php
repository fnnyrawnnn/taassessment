...
<form method="post" id="formsubmit" action="{{ route('session.simpan') }}">
    ...
    @if($key < 5) <div class="container page-{{ (floor(($key / 5)) + 1)  }}" style="display: block;">
        @else
...
<div class="container" style="margin-left: 45%; margin-rigth: 50%; display: block;">
    <nav aria-label="Page navigation example" style="margin: 0 auto;">
        <ul class="pagination">
            ...
            @for($i = 0; $i < count($questionss) / 5; $i++) @if($i==0) <li class="page-item num-page active"><a class="page-link page" id="{{ $i + 1 }}" href="#">{{ $i + 1 }}</a></li>
                @elseif($i > 0)
                <li class="page-item num-page"><a class="page-link page" id="{{ $i + 1 }}" href="#">{{ $i + 1 }}</a></li>
                @endif
                @endfor
                @elseif(count($questionss) % 5 != 0)
                @for($i = 0; $i < floor((count($questionss)) / 5) + 1; $i++) @if($i==0) <li class="page-item num-page active"><a class="page-link page" id="{{ $i + 1 }}" href="#">{{ $i + 1 }}</a></li>
                    @elseif($i > 0)
                    <li class="page-item num-page"><a class="page-link page" id="{{ $i + 1 }}" href="#">{{ $i + 1 }}</a></li>
                    @endif
                    @endfor
